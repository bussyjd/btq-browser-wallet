...
python3 -m http.server 8080 -d examples   # serves examples/dapp.html for the site-connect shot
```
...
| 1 | 0:10–0:35 | **Create**: Create wallet → password + confirm → the 12 words appear | "New wallet: the password seals the vault first, then the phrase is shown — once. It is never written to storage, and it never leaves the service worker." |
| 2 | 0:35–0:55 | Confirm-three-words screen → Seal → the wallet home with a balance of 0 | "You type three of the words back, and the vault is sealed with PBKDF2 + AES-GCM. From here on the phrase exists only in your head." |
...
| 4 | 1:05–1:30 | **Receive** tab: address `tbtq1z…`, path `m/0'/0'/0'`, QR, Copy | "Receive gives the next unused address, with its derivation path and a QR of that exact string. Copy — that is the address, checked character for character in the tests." |
...
| 7 | 2:00–2:15 | **Settings**: explorer URL, node URL/user/password, **Test** → chain + height, Save → pill reads `Testnet · node` | "The explorer has no broadcast route, so signing and broadcasting are split: read from the explorer, push through your own node. Test tells you if the node is on a different chain than the explorer." |
...
| 9 | 2:50–3:05 | Password field → Confirm → result card: txid, "Broadcast", explorer link; Activity row Pending → confirmed | "The password is asked again, the transaction is signed **before** any network call, and the raw hex is kept even if the broadcast fails. Nothing signs without the leaf committing to the witness program." |
| 10 | 3:05–3:25 | **Second Chrome profile**, load the same `dist/`, Import → paste the 12 words → new password → the same balance and history come back | "Import on a clean profile: same phrase, different password, and the scan walks the gap limit — it finds coins on addresses this wallet had never displayed." |
| 11 | 3:25–3:45 | `localhost:8080/dapp.html` → Connect → the approval window naming the origin → Approve → the account appears on the page; then Settings → Connected sites → Revoke | "Site-connect works like MetaMask: the page sees `window.btq`, asks, and gets nothing until you approve — per origin, revocable, and a denial comes back as 4001." |
...
Cut ruthlessly if it runs long: shots 3 and 6 are the ones to shorten, never
send (12 pts) or import (8 pts).
...
browser and every reviewer's player will open.
