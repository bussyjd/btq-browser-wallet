...
import { parseHistoryResponse } from '../../src/core/explorer/history.js';
...
describe('explorer UTXO/history/broadcast parsers (shipped)', () => {
  it('treats 404 and Address not found as empty, not spendable', () => {
    expect(parseUtxoResponse(404, undefined, ADDR, SCRIPT)).toEqual([]);
    expect(parseUtxoResponse(200, { error: 'Address not found' }, ADDR, SCRIPT)).toEqual([]);
    expect(parseHistoryResponse(404, undefined, ADDR)).toEqual([]);
  });
  ...
  it('parses a live-shaped UTXO after the script matches', () => {
  ...
  it('broadcast 404 is a failure, not a txid', () => {
    expect(() => parseBroadcastResponse(404, { message: 'not found' })).toThrow(/no broadcast route/);
  });
